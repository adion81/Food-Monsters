...
{
    public class VegetableTypeMonster : Monster
    {
        public VegetableTypeMonster (string Name, int Level, int Attack, int Defense, int MaxHP) : base (Name, Level, Attack, Defense, MaxHP) { }

        public override void DoAttack(Monster target)
        {
            if(target is FungusTypeMonster)
            {
